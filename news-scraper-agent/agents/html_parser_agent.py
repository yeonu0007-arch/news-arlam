...
from external.local_scraper.client import LocalScraperClient
...
class HtmlParserAgent:
    def __init__(self, site: SiteDto):
        ...
        if env.PROFILE == "local":
            self.lambda_invoker = LocalScraperClient()
        ...
        match url := self.site.url:
            ...
            case url if "devocean.sk.com" in url:
                return {
                    "url": url,
                    "content_type": "html",
                    "selector": "div.sec-area > ul.sec-area-list01 > li > div a > h3.pc_view",
                }
            case url if "news.naver.com/section/100" in url or "news.naver.com/section/102" in url or "news.naver.com/section/103" in url:
                return {
                    "url": url,
                    "content_type": "html",
                    "selector": "div.sa_text",
                }
            case url if "sports.news.naver.com" in url:
                return {
                    "url": url,
                    "content_type": "html",
                    "selector": "a[class*='Home_link_title']",
                }
            case url if "entertain.naver.com" in url:
                return {
                    "url": url,
                    "content_type": "html",
                    "selector": "a[class*='Home_link_ranking']",
                }
            case url if "news.google.com/rss" in url:
                return {
                    "url": url,
                    "content_type": "xml",
                    "selector": "",
