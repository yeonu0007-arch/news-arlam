...
class CrawlingAgent:
    ...
    def __call__(self, state: SiteState) -> SiteState:
        ...
        try:
            ...
            self.logger.info(f"[{self.site.name}] Crawling starting with {len(compressed_parser_result)} HTML chunks.")
            ...
            self.logger.info(f"[{self.site.name}] Crawling finished. Extracted {len(response.items)} articles.")
