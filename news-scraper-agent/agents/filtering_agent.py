...
class FilteringAgent:
    ...
    def __call__(self, state: SiteState) -> SiteState:
        ...
        try:
            ...
            self.logger.info(f"[{self.site.name}] Filtering starting with {len(crawling_results_with_id)} articles.")
            time.sleep(6)  # Avoid 429 Rate Limit (15 RPM max)
            ...
            self.logger.info(f"[{self.site.name}] Filtering finished. Key articles selected: {len(filtering_result)}")
