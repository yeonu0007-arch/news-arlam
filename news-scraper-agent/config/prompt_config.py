...
"""
    ...
    # sorting llm agent prompt (AI 전용 - 기존 코드 복원)
...
"""
    ...
    CATEGORY_FILTERING_PROMPT = """<역할 및 목표>
해당 카테고리에서 가장 자극적이고 주목할 만한 뉴스 기사를 선별하는 도구입니다. 사람들의 이목을 끌 수 있는 핵심 기사만 추출합니다.

<문맥>
{crawling_result}

<지시문>
1. 주어진 뉴스 목록에서 가장 핵심적이고, 자극적이며, 대중이 주목할 만한 기사를 최대 15개까지 선별하여 JSON 형식으로 출력하세요.
2. 각 기사 객체에는 'title'과 'url' 속성만 포함합니다.
3. 선별 기준 (우선순위 순):
    - 충격적이거나 놀라운 사건/발표
    - 사회적 파급력이 큰 이슈
    - 많은 사람들의 관심을 끌 수 있는 화제성 뉴스
    - 논쟁적이거나 토론을 유발하는 주제
4. 광고성 기사, 단순 홍보 기사, 중요도가 낮은 기사는 반드시 제외합니다.
5. 반드시 최대 15개 이하로 출력하세요.
6. JSON 형식으로 주어진 형식에 맞게 출력하세요.
"""

    CATEGORY_SORTING_PROMPT = """<역할 및 목표>
뉴스 기사를 자극성과 주목도에 따라 정렬하는 도구입니다. 가장 핵심적이고 이목을 끌 만한 기사가 최상위에 오도록 정렬합니다.

<문맥>
{filtering_result}

<지시문>
1. 주어진 뉴스 목록에서 각 기사를 자극성과 주목도에 따라 순서대로 정렬하세요.
2. 가장 핵심적이고 자극적이면서 주목할 수 있는 만한 기사가 1, 2, 3위에 오도록 평가합니다.
3. 정렬 기준 (높을수록 상위):
    - **1순위**: 충격적이거나 긴급한 속보성 뉴스
    - **2순위**: 사회적 파장이 크고 논쟁적인 이슈
    - **3순위**: 대중의 호기심을 자극하는 화제성 뉴스
    - **4순위**: 일상에 직접 영향을 미치는 실용적 정보
4. JSON 형식으로 주어진 형식에 맞게 출력하세요.
    - id: 요청에 포함된 id
    - reason: 이 기사의 주목도 이유 (예: 충격적 사건, 화제성 등)
"""

    # ===== 기후 전용 필터링 프롬프트 =====
    CLIMATE_FILTERING_PROMPT = """<역할 및 목표>
단순한 일기 예보나 생활 날씨가 아닌, 기후 위기에 대한 경각심을 고취시킬 수 있는 핵심적이고 위기감 있는 기사를 선별하는 도구입니다.

<문맥>
{crawling_result}

<지시문>
1. 주어진 뉴스 목록에서 일상적인 날씨나 생활 관련 뉴스는 반드시 제외하고, 기후 위기, 환경 재앙, 지구 온난화의 심각성을 알 수 있는 파급력이 큰 기사만 필터링하여 최대 15개까지 JSON 형식으로 출력하세요.
2. 주요 키워드: '기후 위기', '온난화 심각', '탄소 중립', '이상 기후', '재앙', '가뭄', '홍수', '해수면 상승', '생태계 파괴'
3. 각 기사 객체에는 'title'과 'url' 속성만 포함합니다.
4. 사람들에게 기후적으로 위기관리를 체감하게 하거나 경각심을 줄 수 있는 핵심 기사를 우선 선별합니다.
5. 반드시 최대 15개 이하로 출력하세요.
6. JSON 형식으로 주어진 형식에 맞게 출력하세요.
"""
