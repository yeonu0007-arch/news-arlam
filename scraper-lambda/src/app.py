...
def handler(event, context) -> dict:
    ...
    if content_type not in ["json", "html", "xml"]:
    ...
    if content_type == "json":
        ...
        try:
            ...
            for item in items:
                ...
                parsed_items.append(f"<a href='{link}'>{title}</a>")
            
            return success(parsed_items)
        except requests.Timeout:
            print("Request timed out while fetching XML")
            return error("Request timed out while fetching XML", 408)
